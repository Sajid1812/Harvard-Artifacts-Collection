...
classifications = ["Paintings", "Photographs", "Sculpture", "Prints", "Drawings", "Vessels","Coins","Accessories (non-art)",
                   "Textile Arts","Archival Material","Fragments","Manuscripts","Seals","Straus Materials"]
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS artifact_metadata (
                ...
                accessionyear VARCHAR(32),
                accessionmethod VARCHAR(128),
...
def insert_data_to_db(meta, media, color):
    ...
    try:
        ...
        if not color.empty:
            color_tuples = [
                (
                    c['objectid'], c['color'], c['spectrum'], c['hue'], c['percent'], c['css']
                )
...
def display_efficient(data, name, max_rows=500, full_table=False):
    ...
    if full_table:
        ...
        if len(data) > max_rows:
            ...
            html_code = f"""<div style="height:700px; overflow:auto; border:1px solid #ccc; background:#f9f9f9; padding:10px; font-family:monospace; white-space:pre;">
...
if st.session_state.active_btn == "migrate":
    ...
    if insert_btn:
        if meta is not None and not meta.empty:
            try:
                insert_data_to_db(meta, media, color)
                st.success("Data inserted successfully!")
                st.session_state.inserted_data_all["meta"] = pd.concat([st.session_state.inserted_data_all["meta"], meta], ignore_index=True)
                st.session_state.inserted_data_all["media"] = pd.concat([st.session_state.inserted_data_all["media"], media], ignore_index=True)
                st.session_state.inserted_data_all["color"] = pd.concat([st.session_state.inserted_data_all["color"], color], ignore_index=True)
                st.session_state.display_tables = True
            except pymysql.err.IntegrityError:
                st.error("Insert failed: Data already inserted (duplicate entries).")
            except Exception as e:
                st.error(f"Insert failed: {str(e)}")
        else:
            st.warning("No collected data to insert. Please collect first.")
    ...
    color = st.session_state.collected_data.get("color")
    with col1:
        display_efficient(meta, "Metadata", max_rows=500, full_table=False)
    with col2:
        display_efficient(media, "Media", max_rows=500, full_table=False)
    with col3:
        display_efficient(color, "Color", max_rows=500, full_table=False)
    ...
    if sql_stmt:
        ...
        try:
            ...
            safe_display_dataframe(df, name=f"Result: {queries[selected_query_idx]}")
        except Exception as e:
